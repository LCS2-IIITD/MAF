# Code for Trimodal mBART (MAF-TAV-mBART)
# File: Trimodal-mBART-driver.py

# Run code-mixed file:
# nohup python -u Trimodal-mBART-driver.py > ../out-files/MAF-TAV-mBART.out &
...
MODEL_OUTPUT_DIR = '../models/MAF-TAV-mBART/'
RESULT_OUTPUT_DIR = '../results/MAF-TAV-mBART/'
...
# -------------------------------------------------- Multimodal MultimodalMBartModel --------------------------------------------------
...
# ---------------------------------------------- MultimodalMBartForConditionalGeneration ----------------------------------------------
...
def get_val_scores(model,
    ...
    if "Validation" in desc:
        ...
        file_name = RESULT_OUTPUT_DIR + "val/MAF_TAV_mBART_epoch_" + str(epoch+1) + "_val_results.csv"
        ...
        file_name = RESULT_OUTPUT_DIR + "test/MAF_TAV_mBART_epoch_" + str(epoch+1) + "_test_results.csv"
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        path = MODEL_OUTPUT_DIR + "MAF_TAV_mBART_epoch_"  + TARGET_COLUMN + "_epoch_" + str(epoch+1) + "_" + datetime.now().strftime('%d-%m-%Y-%H:%M')
        print(path)
        save_model(model,
                   path,
                   tokenizer)
